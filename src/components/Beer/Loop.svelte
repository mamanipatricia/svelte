<script>
  ...
  function getArray(ingredients) {
    console.log("Object.keys(ingredients)", Object.keys(ingredients));
    // return [...ingredients.malt, ...ingredients.hops]
    // salida:  [{name: "rerer", value: 1223}, {name: "rerer", value: 1223}]
    const ingredientsTypes = Object.keys(ingredients); // ["malt", "hops", "yeast"]
    // verify if the item is an array and this has to be an object that contain {name, value} properties...

    const maltIndex = ingredientsTypes.indexOf("malt");
    let initialIngredient = ingredients[ingredientsTypes[maltIndex]];

    ingredients.hops.map().map((ingredientHops) => {
      const foundIngredient = initialIngredient.findIndex(
        (item) => ingredientHops.name === item.name
      );
      if (foundIngredient >= 0) {
        initialIngredient[foundIngredient] = {
          ...initialIngredient[foundIngredient],
          amount: {
            value:
              initialIngredient[foundIngredient].amount.value +
              ingredientHops.amount.value,
          },
        };
      } else {
        initialIngredient.push(ingredientHops);
      }
    });
  ...
  <div class="wrapper">
    <div class="description">
      ...
      <div class="ingredients">
        {#each getArray(beer.ingredients) as { name, amount: { value } }}
          <li>{name} : {value}</li>
        {/each}
        <!-- {#each beer.ingredients.hops as { name, amount: { value } }}
          <li>{name} : {value}</li>
        {/each} -->
      </div>
    </div>
